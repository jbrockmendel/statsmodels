...
class TestGradMNLogit(CheckGradLoglikeMixin):
    ...
    def test_hess(self):
        ...
        for test_params in self.params:
            ...
            # FIXME: these just don't work well, dont leave commented-out
    ...
    def test_hess_fun1_fd(self):
        for test_params in self.params:
            ...
            if hetrue is not None:  # Hessian doesn't work for 2d return of fun
                ...
                hefd = numdiff.approx_hess1(test_params, fun,
                                            # TODO: should be kwds,
                                            # e.g. epsilon=1e-8
                                            args=self.args)
                ...
                #    TestDerivativeFun
                hefd = numdiff.approx_hess2(test_params, fun,
                                            # TODO: should be kwds,
                                            # e.g. epsilon=1e-8
                                            args=self.args)
                assert_almost_equal(hetrue, hefd, decimal=DEC3)
                hefd = numdiff.approx_hess3(test_params, fun,
                                            # TODO: should be kwds,
                                            # e.g. epsilon=1e-8
                                            args=self.args)
                assert_almost_equal(hetrue, hefd, decimal=DEC3)
...
class TestDerivativeFun(CheckDerivativeMixin):
    ...
    def hesstrue(self, params):
        # Making this (3,3) because test fails with scalar 0
        return np.zeros((3, 3))
        # TODO: why is precision only DEC3
...
if __name__ == '__main__':
    ...
    print(approx_hess_cs((1, 2, 3), fun, (x,), h=1.0e-20))
    # this hessian is correctly zero
    ...
    g = approx_fprime_cs((1, 2, 3), fun1, (y, x), h=1.0e-20)
